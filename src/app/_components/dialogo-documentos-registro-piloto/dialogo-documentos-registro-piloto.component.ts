...
import { PilotoService, DocumentosService} from '../../_services';
...
@Component({
  selector: 'app-dialogo-documentos-registro-piloto',
  templateUrl: './dialogo-documentos-registro-piloto.component.html',
  styleUrls: ['./dialogo-documentos-registro-piloto.component.scss']
})
export class DialogoDocumentosRegistroPilotoComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    public dialogRef: MatDialogRef<DialogoDocumentosRegistroPilotoComponent>,
  ...
  getDocumentosContrato(idVehiculo: number) {
    ...
    this.pilotoService.getDocumentosPiloto(idVehiculo)
      .pipe(first())
      .subscribe(dataList => {
  ...
  onFileSelected(e: any, row: any) {
    ...
    if (this.isFileAllowedPDF(this.uploadedFiles[0].name)) {
      ...
      this.pilotoService.postGuardaDocumentoRegistroPiloto(formData)
        .pipe(first())
        .subscribe(
          data => {
